...
class Paso2():
	...
	def make_botones(self, homogeneous, spacing, expand, fill, padding):
		...
		def funciones(self):
			...
			if texto1.get_text() == "": 
				...
				gtk.gdk.threads_enter()
				error=gtk.MessageDialog(parent=None, flags=0, type=gtk.MESSAGE_ERROR, buttons=gtk.BUTTONS_CLOSE, message_format="Debe incluir los \"Paquetes Disponibles en el Mirror\" escribiendo el nombre del paquete en el campo de texto y luego dar clic en el botón \"Validar\"")
				error.run()
				error.destroy()
				gtk.gdk.threads_leave()
	...
	def caja2(self, homogeneous, spacing, expand, fill, padding):
		...
		combo.append_text('wheeze')
		combo.append_text('sid')
		combo.append_text('experimental')
	...
	def caja6(self, homogeneous, spacing, expand, fill, padding):
		...
		etiqueta=gtk.Label("Paquetes disponibles en el Mirror:\t")
		caja.pack_start(etiqueta, False, False, 5)
		etiqueta.show()
	...
	def caja7(self, homogeneous, spacing, expand, fill, padding):
		...
		etiqueta=gtk.Label("Paquetes Extas:\t\t\t\t")
		caja.pack_start(etiqueta, False, False, 5)
		etiqueta.show()
		...
		boton.connect("clicked", buscar_isopool_clic)
		caja.pack_start(boton, False, False, 5)
		boton.show()
